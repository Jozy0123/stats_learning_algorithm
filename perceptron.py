...
class Perceptron:
    ...
    def perceptron(weight, bias, features):
        ...
        result = np.matmul(features, weight) + bias
        prediction = np.apply_along_axis(classify, 1, result)
    ...
    def train(self, features, label):
        ...
        bias = 0  # 偏置初始值
        weight = np.zeros(weight_length)[:, np.newaxis]
        ...
        for _ in range(self.steps):
            ...
            if len(np.where(np.array(label_hat.flatten() > 0))[0]) == len(label_hat.flatten()):
                break

            else:
                index = np.where(np.array(label_hat.flatten() <= 0))[0]  ## 找到没预测准的数据
                wrong_result = np.random.choice(index)
